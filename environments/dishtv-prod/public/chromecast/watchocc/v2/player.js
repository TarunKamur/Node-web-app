...
sampleplayer.CastPlayer.prototype.load = function(info) {	
  ...
  if (!media.contentId) {
    ...
    self.showPreviewModeMetadata(false);
    self.displayPreviewMode_ = false;
    self.previewDuration_ = media.customData.previewDuration;   // defined value
    console.log(self.previewDuration_ + " = previewDuration");
...
sampleplayer.CastPlayer.prototype.loadVideo_ = function(info) {
	...
	console.log(info.message.media.contentId)
	console.log("##########this is alert ##########")
	
	
	console.log("Video is loaded "+ info.message.media.contentId)
  ...
  var licenseUrl = info.message.media.customData.licenseUrl;
  ...
  if (!protocolFunc) {
    ...
    if (!this.preloadPlayer_ || (this.preloadPlayer_.getHost &&
      ...
      this.log_('loadVideo_1: licenseUrl: ' + licenseUrl);
      if (licenseUrl) {
      	this.log_('loadVideo_2: licenseUrl: ' + licenseUrl);
       	host.licenseUrl = licenseUrl;
       	host.licenseCustomData = licenseUrl;//info.message.media.customData;
      }
...
sampleplayer.CastPlayer.prototype.onPlaying_ = function() {
  ...
  if(!!this.previewDuration_){                      
      var count = 0;
      var interval = setInterval(function(){ 
        console.log(count + ': _count');
        count += 1;
        if(count >= self.previewDuration_){
          console.log('stoped');           
          self.receiverManager_.stop();
    ...
    if (!isNaN(curTime) && !isNaN(totalTime)) {
      ...
      if (this.displayPreviewMode_) {
        this.previewModeTimerElement_.innerText = "" + Math.round(totalTime-curTime);
			  ...
			  this.totalTimeElement_.textContent = 'LIVE';
...
sampleplayer.CastPlayer.prototype.onLoadSuccess_ = function() {
  ...
  // and progress bar
  var totalTime = this.mediaElement_.duration;
  console.log("totalTime totalTime totalTime "+ totalTime)
...
sampleplayer.getType_ = function(media) {
  ...
  console.log("sampleplayer.getExtension_(contentUrlPath) : "+sampleplayer.getExtension_(contentUrlPath))
